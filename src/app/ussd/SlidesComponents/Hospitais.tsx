...
const Hospitais: React.FC<HospitaisProps> = ({
  provinceName,
  municipeName,
  setNumberSelected,
}) => {
  ...
  const [nearHospitals, setNearHospitals] = useState<NearHospitals[]>([]);
  ...
  const sendState = React.useCallback(async () => {
    try {
      ...
      const response = await axios.post(
        "https://mapazzz.onrender.com/api/danger_zone/sendHelpHospital",
        {
          address: "Angola, " + provinceName + ", " + municipeName,
        },
        {
          headers: {
      ...
      setLoading(false);
      setNearHospitals(response.data);
    } catch (error) {
    ...
    <div className="flex gap-2 flex-col items-center justify-start py-5 h-full w-full">
      <section className="flex flex-col h-full justify-between w-full">
        <header className="flex flex-col gap-1 items-center pb-2  justify-start w-full">
          <h3 className="text-zinc-100 font-medium">Hospitais mais Próximos</h3>
        ...
        <footer className="my-0 w-full px-1 h-full overflow-y-auto flex flex-col justify-between">
          <div className="h-full overflow-y-auto">
            ...
            {!loading && nearHospitals.length > 0 ? (
              nearHospitals.map((hospital, index) => (
